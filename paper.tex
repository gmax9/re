...
As we encountered in our prior work~\cite{2024gao} while devising assessments used to compare a baseline method against ours,
validation of detection results is not straightforward in the absence of curated data, implementations are not guaranteed to be replicable, and meaningful metrics require alignment.

In this report, we survey an extensive body of literature that propose darknet event detection methodologies to investigate the extent that the challenges we encountered apply to comparative method assessments at large. 
...
Section 2 introduces our taxonomy of detection methodologies, summarizing intuitions guiding the design of each method while emphasizing their distinguishing features. 
Section 3 reviews empirical assessments of prior work separate from methods themselves to highlight gaps that prevent rigorous comparative assessments.
Finally, we conclude our survey's findings with directions for future work that addresses these gaps to enable...
...
Researchers have studied this unidirectional traffic to understand its mixtures and origins with foundational work attributing its causes to Internet-wide activity that includes
...
\begin{table}[t]
    ...
    \setlength{\tabcolsep}{3pt} % Reduced column padding for better fit with 15 columns
    \begin{tabular}{lcccccccccccccc} % 15 columns total: 1 + 4 + 6 + 4
        \toprule
        % Top Header Row - 15 columns, no vertical or horizontal dividers
        \multicolumn{1}{c}{\multirow{2}{*}{\textbf{Work}}} & 
        \multicolumn{4}{c}{\textbf{Activities}} & 
        \multicolumn{6}{c}{\textbf{Techniques}} & % Techniques now spans 6 columns
        \multicolumn{4}{c}{\textbf{Traffic Representation}} \\
        \cline{2-15} % Adjusted cline to span 2 to 15
        
        % Sub Header Row with Rotation - 14 ampersands total
        & \rot{90}{Scanning} & \rot{90}{DDoS} & \rot{90}{Outage} & \rot{90}{Misconfiguration} 
        & \rot{90}{Clustering} & \rot{90}{Dimensionality Reduction} & \rot{90}{Forecasting} 
        & \rot{90}{Thresholding/Fingerprinting} % New Column (H)
        & \rot{90}{Representation Learning} % New Column (I)
        & \rot{90}{Frequent Pattern Mining} % New Column (J)
        & \rot{90}{Graph} & \rot{90}{Feature Vector} & \rot{90}{Sequences} & \rot{90}{Time Series}\rule{0pt}{12em} \\ 
        \midrule

        Evrard et al.~\cite{2019evrard} & $\checkmark$ & & & & $\checkmark$ & & & & & & $\checkmark$ & & & \\

        Lagraa et al.~\cite{2017lagraa,2019lagraa} & $\checkmark$ & & & & $\checkmark$ & & & & & & $\checkmark$ & & & \\

        Kallitsis et al.~\cite{2022kallitsis} & $\checkmark$ & $\checkmark$ & & & $\checkmark$ & $\checkmark$ & & $\checkmark$ & & & & $\checkmark$ & & \\

        Iglesias et al.~\cite{2019iglesias} & $\checkmark$ & $\checkmark$ & & & $\checkmark$ & & & $\checkmark$ & & & & $\checkmark$ & & \\

        Nishikaze et al.~\cite{2015nishikaze} & $\checkmark$ & & & & $\checkmark$ & & & & & & & $\checkmark$ & & \\

        Soro et al.~\cite{2020soro} & $\checkmark$ & & & & $\checkmark$ & & & & & & & & $\checkmark$ & \\

        Gioacchini et al.~\cite{2021gioacchini,2023gioacchini} & $\checkmark$ & & & & $\checkmark$ & $\checkmark$ & & & & & & & $\checkmark$ & \\

        Abduaziz et al.~\cite{2025} & $\checkmark$ & & & & $\checkmark$ & $\checkmark$ & & & & & & & $\checkmark$ & \\

        Han et al.~\cite{2021han,2022han} & $\checkmark$ & & & & & $\checkmark$ & & & & & & & & $\checkmark$ \\

        Han et al.~\cite{2020han,2022han} & $\checkmark$ & & $\checkmark$ & $\checkmark$ & & $\checkmark$ & & & & & & & & $\checkmark$ \\

        Kanehara et al.~\cite{2019kanehara,2022han} & $\checkmark$ & & & & & $\checkmark$ & & $\checkmark$ & & & & & & $\checkmark$ \\

        Kartsioukas et al.~\cite{2023kartsioukas} & $\checkmark$ & $\checkmark$ & & & & $\checkmark$ & & $\checkmark$ & & & & & & $\checkmark$ \\

        Ban et al.~\cite{2016ban} & $\checkmark$ & & & & & $\checkmark$ & & & & & & & $\checkmark$ & $\checkmark$ \\ 

        Torabi et al.~\cite{2020torabi,2018torabi} & $\checkmark$ & & & & & & & & & & $\checkmark$ & $\checkmark$ & & \\

        Tanaka et al.~\cite{2023tanaka,2021tanaka} & $\checkmark$ & & & & & & & & & & $\checkmark$ & $\checkmark$ & $\checkmark$ & \\

        Niranjana et al.~\cite{2019niranjana} & $\checkmark$ & & & & $\checkmark$ & $\checkmark$ & & & & & $\checkmark$ & $\checkmark$ & $\checkmark$ & \\

        Cabana et al.~\cite{2019cabana} & $\checkmark$ & & & & $\checkmark$ & & & $\checkmark$ & & & $\checkmark$ & $\checkmark$ & & \\

        Shaikh et al.~\cite{2018shaikh} & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & & & & & & & & $\checkmark$ & & \\
  
        Zakroum et al. (A)~\cite{2022zakroum,2018zakroum} & $\checkmark$ & & & & $\checkmark$ & & $\checkmark$ & & & & & & & $\checkmark$ \\
        Zakroum et al. (B)~\cite{2023zakroum} & $\checkmark$ & & & & $\checkmark$ & & $\checkmark$ & & & & & & & $\checkmark$ \\

        Guillot et al.~\cite{2019guillot} & & & $\checkmark$ & $\checkmark$ & $\checkmark$ & & $\checkmark$ & & & & & & & $\checkmark$ \\

        Enyanet et al.~\cite{2024enyanet} & & & $\checkmark$ & $\checkmark$ & $\checkmark$ & & $\checkmark$ & & & & & & & $\checkmark$ \\
...
Some methods such as~\cite{2022kallitsis} are capable of detecting multiple types of activities depending on whether their input traffic is filtered.
...
\begin{table*}[t!]
    ...
    \caption{Empirical assessment details and darknet traffic datasets found in our surveyed works. Table~\ref{tab:telescopes} lists expanded details of the telescopes referenced in this table.}
    \label{tab:eval}
...
\begin{figure}[ht!]
    ...
    \caption{Time frames and darknet sources of datasets used in detection methodology assessments in our surveyed works shown with dates of darknet-observable Internet-wide events.}
    \label{fig:traffic_growth}
\end{figure}
...
Empirical studies such as~\cite{2025camargo,2019soro} establish that these factors have a non-trivial impact on the observability of darknet traffic (\textit{i.e.,} in terms of traffic composition and visibility of sources).
...
The majority of works do not provide clear rationale for their selection of time frames (with some exceptions such as ~\cite{2022kallitsis} which choose
