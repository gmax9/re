...
Empirical studies~\cite{@@} have found that these factors have a non-negligible impact on
the observability of senders. %TODO: reword

Across our surveyed works, we identified a total of 7 darknets (listed in Table~\ref{@@}) from which datasets source their traffic. 
...
While these numbers represent the most up-to-date sizes reported by operators, darknet address space in practice may fluctuate based on ad-hoc announcements (e.g., for leasing purposes~\cite{@@}) or in the case of more permanent changes such as address ownership transfers~\cite{2019ardc}). 
...
%Recompute
Most datasets are short snapshots: over 50\% of all datasets span fewer than two months, while only a third draw from multi-year darknet traffic, and only one in five cover three or more years.
While a majority of works do not provide clear rationale for their selected time frames, some exceptions such as ~\cite{@@} and ~\cite{@@} do choose their time frames to assess specific events (e.g., Mirai and anomalous TCP-port 0 scans). 
Still, coverage overlap with major Internet-wide scanning events pictured in Figure~\ref{@@} is incidental and their detectability by methodologies inconclusive.
% Recompute
...
\textbf{Takeaways:} 
Prior work assess detection methodologies on datasets of differing degrees of observability given their formation using 
traffic sourced from a variety of darknets and time periods.
% Prior work assess detection methodologies on datasets formed using traffic sourced 
% from a variety of darknets that span different time periods and lengths and thus represent different degrees 
% of observability.
Generalizability of assessment results for specific methods remains unclear and prohibits indirect comparisons 
using individual method performance results drawn from differing datasets.}
...
\textbf{Takeaways from Findings:} helloasdfasdfasdf
...
Other works such as ~\cite{@@} rely on Java or C, which have become less popular as preferences shift towards higher-languages for analytics use-cases.
...
Across our surveyed works, hardware release dates trail publication dates by as much as 10 years.
