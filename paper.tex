...
\title{A Survey of Detection Methodologies for Darknet Scanning Activities: Design, Implementation, and Assessment}
\author{Max Gao}
...
\begin{abstract}
Network telescope, or darknet, traffic has been instrumental in providing visibility into Internet-wide activities such as malicious scanning, denial-of-service (DoS) backscatter, and outages.
To better operationalize darknets for their monitoring capabilities, researchers have proposed a number of automated detection methodologies that consist of a diverse set of techniques 
which operate on a variety of features derived from darknet traffic.
Yet despite an abundance of methods, less attention has been devoted towards their comparative assessment, resulting in a lack of complete empirical evidence needed to establish trade-offs 
in their detection capabilities and computational performance.
In this report, we survey works from the past decade that propose methods to detect darknet scanning activity and review components of their design, implementation, and, with emphasis,
their assessments to identify key challenges that hinder their comprehensive assessment.
We conclude with a discussion of systematic practices to enable comparative assessment so that future work may yield measurable improvements in method development.

% to for future work to enable systematic comparisons that 
% that may yield improvements in method development by enabling systematic comparisons.

% with an emphasis on assessment practices in order to identify challenges to their
% comprehensive comparative assessment.

% This report surveys works that propose methods designed for darknet scanning activity with an emphasis on their empirical assessment practices to identify the specific challenges that hinder 
% In this report, we survey prior works which propose methods designed for darknet scanning activity and study 
...
Network telescopes, or darknets, have been instrumental to both researchers and practitioners for 
their ability to observe Internet-wide activities in the unsolicited traffic they receive.
Over the past two decades, research efforts have shifted from characterizing the signals of these 
activities towards translating such empirical insights into operationalizable methodologies for 
monitoring the Internet's security and availability in near-real time.
These efforts have culminated into a still-growing number of methods that, despite sharing the broad goal of automatic and accurate detection, 
differ widely in their formulations of detection tasks (\textit{e.g}., source classification versus target identification), 
the technique(s) they employ (\textit{e.g.,} dimensionality reduction, forecasting, representation learning), 
and the features of the traffic they exploit (\textit{e.g.}, packet inter-arrival times, destination port sequences).
However, as many of these methods have been developed, implemented, and assessed independently by different research groups,
less effort have been directed towards ensuring their comparability, much less a comprehensive comparison of their real-world capabilities. 
Consequently, researchers lack convincing empirical evidence to distinguish trade-offs between existing methods, especially those which increasingly 
rely on modern machine-learning techniques, and thus, it is unclear which methods represent substantial advancements among others and where weaknesses 
of existing methods can be improved.
As we encountered in our prior work~\cite{2024gao}, the lack of a systematic approach further discourages the design and assessment of new methods as 
absent standards ambiguate measurable progress.

In this report, we survey an extensive body of works that propose detection methodologies that target scanning activities in darknet traffic. 
We structure our survey into three parts that focus separately on the design of these methods, their empirical assessments, and the specific challenges 
to their comprehensive assessment that we identified.
Section~\ref{sec:methods} provides a taxonomy of canonical methods and discusses motivations for their use from a technical perspective.
Section~\ref{sec:assessments} reviews the components of empirical assessments employed by prior works to demonstrate method efficacy.
Section~\ref{sec:challenges} consolidates our findings on method assessments and elaborates on how they pose as practical challenges to comprehensive assessment.
We conclude this report with a discussion of practices that can guide future work on detection methods towards a more systematic assessment approach.

% We review the motivations and technicalities behind their design separately from their empirical assessments.
...
\section{Assessments of detection methods}
\label{sec:assessments}

Prior works aim to demonstrate the utility of their methods through empirical assessments that yield evidence of a method's detection capabilities and computational performance under realistic traffic conditions.
In this section, we discuss the reproducibility and comparability of these assessments as influenced by three core components:
1) darknet traffic datasets;
2) strategies used to validate method outputs; and
3) method implementations and computing environments used to run assessments. 
...
\begin{table}[]
	\small
	\caption{
        Summary of network telescopes referenced in our surveyed works. 
        Though historical telescope sizes and availability of their traffic data may vary across assessments, listed values reflect the most recently reported information.
...
\begin{figure}[!htbp]
    ...
    \captionof{table}{Historic landmark events either purported or confirmed to have been observed by darknets.}
    \label{tab:events}
...
\label{sec:assess:validation}

Due to a lack of definitive "ground truth" for attributing darknet traffic to its root causes, prior works rely on ad-hoc validation strategies to establish efficacy of their methods.
These strategies consist of different practices and as a result, vary in their plausibility, reproducibility, completeness, and cost. 
We examine these qualities for each strategy by focusing on three components (summarized in Table~\ref{tab:eval}) shared across surveyed works:
1) the use of labeled data;
2) the incorporation of external information sources; and
3) comparisons against baseline methods.

% Validation of method outputs against labeled data produces comparable, objective metrics and furthermore are essential outside of validation for methods that depend on model training.
Roughly half of our surveyed works validate their method outputs against labeled data with a majority that derive 
label definitions from telescope-external information sources.
A subset of these works~\cite{2025abduaziz,2024huang,2021gioacchini,2023gioacchini,2022kallitsis}
reuse an identical set of definitions (\textit{i.e.}, addresses that belong to self-disclosed scanning organizations~\cite{2023collins}, 
Mirai-like packet fingerprints~\cite{2017antonakakis}) for both validation and training of models used by their methods.
Since their definitions are formalized, labels are programmatically transferrable across datasets and require low amounts of 
manual effort to achieve an acceptable degree of coverage.
However, the accuracy of mappings between IP addresses and their organizations can change over time due to network and infrastructure churn
and thus, synchronization of definitions with prefix ownership records~\cite{prefix2as} is necessary to preserve label integrity.
In contrast, another subset of works~\cite{2023dandrea,2023zakroum,2019bouharb} avoid directly labeling their darknet traffic datasets 
and instead cross-reference the labels provided through an external data source, 
\textit{e.g.,} AbuseIPDB~\cite{abuseipdb} for crowdsourced threat-intelligence reports, DShield~\cite{dshield} for aggregated-sensor feeds.
While this likewise incurs low cost in terms of manual efforts, the plausibility of their validation outcomes depends on 
data curation practices of the external data provider.
Although validation strategies used by ~\cite{2022han,2021han,2020han,2017ban} also leverage labeled data,
their labels are either a subset of post-hoc corroborated event detections or based on loose definitions that cannot be easily transferred.
In either case, both entail higher costs of manual efforts by domain experts and may contain higher degrees of observational error than 
strategies employed by the first two groups of works.

9 out of the 13 previously mentioned work employ direct comparisons with baseline methods that address the same detection tasks 
as their proposed methods and typically report improvements in detection accuracy and computational performance trade-offs for cases 
where comparisons are straightforward to quantify.
The other 4 works~\cite{2020cohen,2024gao,2022zakroum,2024griffioen} conduct baseline comparisons without relying on explicit labels
though practically their practices differ little with ~\cite{2022han,2021han,2020han,2017ban} as both rely on domain-expertise to 
manually distinguish correct detections.

Finally, the remaining works~\cite{2020soro,2019lagraa,2018zakroum,2017lagraa,2019kanehara,2016ban,2019iglesias,2019niranjana,2015nishikaze}
lack the three components we surveyed as the goal of their assessments is not to provide extensive validation of method performance but instead 
to prove out exploratory analyses that support further development of their methods. 
As a consequence, their strategies are similar to previously mentioned works that rely on post-hoc validation practices, albeit applied at 
smaller scales, \textit{e.g.,} one or two case studies that yield findings linked to historical botnets~\cite{linuxmoose} and 
remotely exploitable vulnerabilities~\cite{redis,memcached}.
Though included along with this group, one exception~\cite{2019iglesias} stands out with 1,634 manually classified alerts.
...
\mybox{Takeaways and Findings}{green!40}{green!10}{
\textbf{Takeaways:}
    Barring works whose assessments are limited to exploratory analyses, roughly half of works employ at least one of the three practices 
    as part of their validation strategies. Validation for representation-learning methods is especially comprehensive and can serve as a 
    standard for best practices.
}

\subsection{Method implementations and computing environments}
\label{sec:assess:env}
In addition to accessible darknet traffic datasets, credible reproduction of method assessments (and application of existing methods to novel traffic datasets) depends on replicable method implementations executed on comparable computing environments.

Only 3 works provide publicly accessible source code of their implementations and roughly half detail the specifications of their computing environments used to run experiments.
Of readily available source code~\cite{2021gioacchini,2022han,2024gao}, we found Python and R as the programming languages of choice.
Both are popular among numeric and scientific communities for implementing analytic workflows due to their abundance of open-source algorithmic libraries which can overall benefit assessment reproducibility depending on researcher skillsets.
Other works implement their methods~\cite{2019lagraa,2014bouharb} in Java or C, which have become less popular as preferences shift towards higher-level languages for analytics use-cases.

Computing environments used to run experiments range from personal laptop workstations~\cite{2025abduaziz,2017lagraa,2019lagraa} to small-scale research compute clusters~\cite{2022han,2019kanehara,2023tanaka,2019iglesias,2019bouharb,2022kallitsis,2021gioacchini,2022zakroum,2023dandrea,2023zakroum}.
Across cluster configurations, physical node counts do not exceed five servers. Individually, these nodes are equipped with at most 256GB of memory.
Most implements execute strictly on CPUs though the subset of works~\cite{2022kallitsis,2021gioacchini,2022zakroum,2023zakroum,2023dandrea} that leverage representation-learning techniques require GPUs for model training purposes.
Nonetheless, release dates for hardware of these computing environments trail publication dates by a minimum of 3 years and at most, 10 years. 

\mybox{Takeaways and Findings}{green!40}{green!10}{

\textbf{Takeaways:} 
    A vast majority of our works do not provide publicly accessible source code for their method implementations which severely limits 
    the relicability of methods to existing or new traffic datasets.
    However, more than half provide details of computing environments that operate using accessible hardware.
}
...
Combined findings of Section~\ref{sec:assessments} indicate limited replicability of the method assessments in prior work 
as a majority do not provide either source code of their implementation, specifications of their computing environments,
nor historical darknet traffic data they used.
Under the most stringent definition of replicability, published work that includes all three components enables 
complete reproduction of its assessment. Across all groups of works, representation-learning showcased the highest fraction (20\%)
of completely reproducible assessments.
Under a less stringent definition that ignores the availability of historical darknet traffic data though still allows for
assessment of a method on previously unassessed data, this fraction increases to 40\% for both 
representation-learning and time series groups.
...
Findings from Section~\ref{sec:assess:validation} show that slightly more than half of surveyed works feature comparative assessments 
against at least one baseline method as a means of validation.
To further quantify the coverage of direct comparative assessments, 
we consider all unique methods, both proposed and baseline, mentioned within a group of works and 
calculate the fraction of direct pairwise comparisons 
out of all possible combinations.
Visualized in Figure -1, the percentages for each respective group, 
23.8\%, 0\%, 9.5\%, 0\%, and 9.5\%, indicate few direct comparative assessments with a significant fraction remaining
to achieve comprehensive comparative assessment of the methods we surveyed.
